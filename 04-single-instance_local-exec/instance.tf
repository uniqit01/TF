...
resource "null_resource" "provision" {
  ...
  provisioner "local-exec" {
    command = "while $(telnet ${openstack_compute_instance_v2.spetrovk-vm.network[0].fixed_ip_v4} 22 </dev/null);do sleep 1;done && echo -e \"[server]\n${openstack_compute_instance_v2.spetrovk-vm.network[0].fixed_ip_v4} ansible_ssh_private_key_file=~/.ssh/id_rsa ansible_ssh_common_args='-o StrictHostKeyChecking=no' ansible_ssh_user=linux\" > inventory-test &&  ansible all -i inventory-test -m setup"
  }
}
